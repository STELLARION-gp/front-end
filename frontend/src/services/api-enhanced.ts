...
class ApiService {
  ...
  async validateUserSession(firebaseUser: {
    uid: string;
    email: string | null;
    ...
    try {
      ...
      if (profile) {
        console.log(
          "✅ Backend session validation successful (via profile fetch)"
        );
        return profile;
      } else {
  ...
  private saveSyncQueueToStorage() {
    try {
      localStorage.setItem(
        "stellarion_sync_queue",
        JSON.stringify(this.pendingSyncQueue)
      );
    } catch (error) {
  ...
  private loadSyncQueueFromStorage() {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `📋 Loaded ${this.pendingSyncQueue.length} items from sync queue`
        );
      }
    } catch (error) {
  ...
  async processSyncQueue() {
    ...
    console.log(
      `🔄 Processing sync queue: ${this.pendingSyncQueue.length} items`
    );
    ...
    for (let i = operations.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        this.pendingSyncQueue.splice(
          this.pendingSyncQueue.indexOf(operation),
          1
        );
    ...
    console.log(
      `✅ Sync queue processed. Remaining items: ${this.pendingSyncQueue.length}`
    );
    this.syncInProgress = false;
  }
