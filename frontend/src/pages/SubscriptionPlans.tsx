...
const SubscriptionPlans: React.FC = () => {
    ...
    const handleFreePlanUpgrade = async (plan: SubscriptionPlan) => {
        try {
            const token = await user?.getIdToken();
            const response = await fetch(`http://localhost:5000/api/subscriptions/user/${user?.uid}`, {
                method: 'PUT',
            ...
            {userSubscription && (
                ...
                >
                    <div className="bg-gray-800 rounded-2xl p-6 border border-gray-700">
                        <h3 className="text-xl font-semibold text-white mb-3">Current Subscription</h3>
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-lg text-blue-400 font-medium">{userSubscription.plan_name}</p>
                                <p className="text-gray-400">{userSubscription.plan_description}</p>
                                {userSubscription.chatbot_questions_limit > 0 && (
                                    <p className="text-sm text-gray-500 mt-2">
                                        Chatbot usage: {userSubscription.chatbot_questions_used}/{userSubscription.chatbot_questions_limit} questions today
                                    </p>
                                )}
                            </div>
                            <div className="text-right">
                                ...
                                {userSubscription.subscription_end_date && (
                                    <p className="text-sm text-gray-500 mt-1">
                                        Expires: {new Date(userSubscription.subscription_end_date).toLocaleDateString()}
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <div className="p-8">
                            ...
                            <div className="text-center mb-8">
                                <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r ${getPlanGradient(plan.plan_type)} mb-4`}>
                                ...
                                <div className="mb-6">
                                    {plan.price_lkr === 0 ? (
                                        <div className="text-3xl font-bold text-white">Free Forever</div>
                                    ) : (
                                        <div>
                            ...
                            <div className="space-y-3 mb-8">
                                {getLocalizedPlan(plan).localizedFeatures.map((feature, featureIndex) => (
