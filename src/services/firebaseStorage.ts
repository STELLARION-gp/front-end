...
export class FirebaseStorageService {
  ...
  static readonly DEFAULT_BLOG_IMAGE =
    "https://charleslittleton.wordpress.com/wp-content/uploads/2022/01/cropped-pexels-photo-110854-1.jpeg";
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("storage/unauthorized")) {
          throw new Error(
            "Upload failed: Not authorized. Please check Firebase Storage rules."
          );
        } else if (error.message.includes("storage/canceled")) {
          throw new Error("Upload was canceled");
        } else if (error.message.includes("storage/unknown")) {
          throw new Error(
            "Upload failed: Unknown error. Please check Firebase configuration."
          );
        } else {
  ...
  static async uploadMultipleImages(
    ...
    const uploadPromises = files.map((file) =>
      this.uploadBlogImage(file, blogId)
    );
