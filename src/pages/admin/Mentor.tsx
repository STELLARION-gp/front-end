...
const Mentor = () => {
  ...
  if (!hasAnyRole(["admin", "moderator"])) {
    return (
      <div className="text-center text-red-500 mt-8">
        {t("common.unauthorized", "You are not authorized to view this page.")}
      </div>
    );
  }
  ...
  return (
    <div className="dashboard-page w-full p-0">
      <div className="flex justify-between items-center mb-6 px-8 pt-8">
        <div>
          <h2 className="text-2xl font-bold mb-1">
            {t("mentorManagement.title", "Mentors")}
          </h2>
          <p className="text-gray-600 dark:text-gray-300 text-sm">
            {t(
              "mentorManagement.listDesc",
              "Manage and view all mentors in the system."
            )}
          </p>
        </div>
        <Button
      ...
      {loading && (
        <div className="px-8">{t("common.loading", "Loading...")}</div>
      )}
        ...
        <table className="min-w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 text-left">
                {t("mentorManagement.phoneNumber", "Phone Number")}
              </th>
              <th className="px-4 py-2 text-left">
                {t("mentorManagement.country", "Country")}
              </th>
              <th className="px-4 py-2 text-left">
                {t("mentorManagement.maxMentees", "Max Mentees")}
              </th>
              <th className="px-4 py-2 text-left">
                {t("mentorManagement.actions", "Actions")}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            {mentors.length === 0 && !loading && (
              <tr>
                <td colSpan={6} className="text-center py-4 text-gray-500">
                  {t("mentorManagement.noMentors", "No mentors found.")}
                </td>
              </tr>
            )}
          </tbody>
