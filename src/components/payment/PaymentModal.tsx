...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const handlePayment = async () => {
    try {
      ...
      // Check if PayHere is available with retry mechanism
      let payhere = (window as any).payhere;
      if (!payhere) {
        console.warn("PayHere not loaded yet, waiting...");
        // Wait up to 3 seconds for PayHere to load
      ...
      if (!payhere) {
        console.error("PayHere object not found on window:", window);
        throw new Error(
          "PayHere payment gateway could not load. Please refresh the page and try again."
        );
      }

      console.log("PayHere object found:", payhere);
      ...
      if (missingFields.length > 0) {
        throw new Error(
          t("subscription.payment.missingFields", {
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">
              {t("subscription.payment.title", {
                planName: localizedPlan.localizedName,
              })}
            </h2>
            <p className="text-gray-400">
              {localizedPlan.localizedDescription}
            </p>
          </div>
          ...
          <div className="mb-6">
            <h3 className="text-lg font-medium text-white mb-3">
              {t("subscription.payment.includedFeatures")}
            </h3>
            <div className="space-y-2">
              {localizedPlan.localizedFeatures.map(
          ...
          {error && (
            <div className="mb-4 p-3 bg-red-500/20 border border-red-500 rounded-lg">
              <div className="text-red-400 text-sm font-medium mb-1">
                {t("subscription.payment.errorTitle")}
              </div>
          ...
          <div className="space-y-3">
            ...
            >
              {loading ? (
                ...
                t("subscription.payment.payButton", {
                  amount: localizedPlan.localizedPrice,
                })
              )}
            </button>
