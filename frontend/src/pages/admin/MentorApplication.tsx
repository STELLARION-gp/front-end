...
const MentorApplication = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (res.status === 201) {
                setSuccess(t('mentorManagement.applicationSuccess', 'Mentor application submitted successfully.'));
                setForm(initialForm);
            } else {
                setError(t('mentorManagement.applicationError', 'Failed to submit mentor application.'));
            }
        } catch (err) {
            setError(t('mentorManagement.applicationError', 'Failed to submit mentor application.'));
        } finally {
        ...
        <div className={styles['guide-app-bg']}>
            <div className={styles['guide-app-container']}>
                ...
                <p className={styles['guide-app-desc']}>
                    {t('mentorManagement.applicationDesc', 'Fill in the details to add a new mentor.')}
                </p>
                <form onSubmit={handleSubmit} className={styles['guide-app-form']}>
                    ...
                    />
                    <div>
                        <label className={styles['guide-label']}>{t('mentorManagement.profileBio', 'Profile Bio')}</label>
                        <textarea
                            id="profile_bio"
                            name="profile_bio"
                            ...
                            rows={3}
                            placeholder={t('mentorManagement.profileBioPlaceholder', 'Profile Bio')}
                        />
                    </div>
                    <InputField
                        label={t('mentorManagement.educationalBackground', 'Educational Background')}
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className={styles['guide-textarea']}
                            rows={3}
                            placeholder={t('mentorManagement.motivationStatementPlaceholder', 'Motivation Statement')}
                        />
                    </div>
                    <div>
