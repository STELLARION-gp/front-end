...
export interface TourRecord extends TourMeta {
  tour_id: number;
  media_ids: number[];
  created_at?: string;
  updated_at?: string;
...
function extractAxiosError(err: unknown): never {
  if (axios.isAxiosError(err)) {
    ...
    throw new Error(
      `Upload failed${status ? ` (HTTP ${status})` : ""}${serverMsg}`
    );
  }
  throw err instanceof Error ? err : new Error("Unknown upload error");
}

export async function uploadSingleTour(
  file: File,
  meta: TourMeta,
  ...
  try {
    ...
    logDebug("uploadSingleTour file", {
      name: file.name,
      size: file.size,
      type: file.type,
    });
    ...
    return (
      await axios.post(`${API_BASE}/tours/upload-single`, form, {
        headers,
...
export async function uploadAlbumTour(
  files: File[],
  meta: TourMeta,
  onProgress?: (p: number) => void
) {
  try {
    ...
    logDebug(
      "uploadAlbumTour files",
      files.map((f) => ({ name: f.name, size: f.size, type: f.type }))
    );
    const headers = await getAuthHeader();
    return (
      await axios.post(`${API_BASE}/tours/upload-album`, form, {
        headers,
        onUploadProgress: (evt) => {
          if (evt.total) {
            const pct = Math.round((evt.loaded / evt.total) * 100);
...
export async function updateTour(id: number, data: Partial<TourMeta>) {
  ...
  return axios
    .put(`${API_BASE}/tours/${id}`, data, { headers })
    .then((r) => r.data);
}
...
export async function deleteTour(id: number) {
  const headers = await getAuthHeader();
  return axios
    .delete(`${API_BASE}/tours/${id}`, { headers })
    .then((r) => r.data);
}
