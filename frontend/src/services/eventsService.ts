...
export async function createEvent(
  ...
  try {
    if (hasFiles) {
      ...
      if (import.meta.env.DEV)
        console.debug("[createEvent] json payload:", rest);
      res = await fetch(`${API_BASE}/events`, {
        method: "POST",
  ...
  if (!res.ok || j.success === false) {
    const msg =
      j.message || j.error || `Failed to create event (status ${res.status})`;
    throw new Error(msg);
  }
...
export async function moderateEvent(id: number, action: "approve" | "reject") {
  ...
  const json = await res.json();
  if (!res.ok || json.success === false)
    throw new Error(json.message || "Failed to moderate event");
  return json;
}
