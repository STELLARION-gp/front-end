...
export const pollService = {
  ...
  async getActivePolls(
    filters?: Omit<PollFilters, "is_active">
   ...
   * Vote on a poll
   */
  async voteOnPoll(pollId: number, choice: string): Promise<VoteResponse> {
    // Changed to string
  ...
  async getPollComments(
    ...
    const url = `/polls/${pollId}/comments${
      queryParams.toString() ? `?${queryParams.toString()}` : ""
    }`;
   ...
   */
  async checkUserVote(pollId: number): Promise<string | null> {
    // Changed to string
    try {
