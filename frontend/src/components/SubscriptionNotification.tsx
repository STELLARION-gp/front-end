...
const SubscriptionNotification: React.FC = () => {
    ...
    const checkShouldShowNotification = (info: SubscriptionInfo) => {
        ...
        if (info.subscription_plan === 'starseeker' && 
            info.chatbot_questions_used >= Math.floor(info.chatbot_questions_limit * 0.8)) {
            setIsVisible(true);
            return;
        }
        ...
        if (info.subscription_status === 'expired' || info.subscription_status === 'cancelled') {
            setIsVisible(true);
            return;
        }
    };
    ...
    const getNotificationContent = () => {
        ...
        if (subscriptionInfo.subscription_plan === 'starseeker' && 
            subscriptionInfo.chatbot_questions_used >= Math.floor(subscriptionInfo.chatbot_questions_limit * 0.8)) {
            return {
                type: 'warning',
        ...
        if (subscriptionInfo.subscription_end_date && subscriptionInfo.subscription_plan !== 'starseeker') {
            ...
            if (daysUntilExpiry <= 3 && daysUntilExpiry > 0) {
                return {
                    ...
                    message: `Your ${subscriptionInfo.subscription_plan.replace('_', ' ')} plan expires in ${daysUntilExpiry} day${daysUntilExpiry !== 1 ? 's' : ''}.`,
        ...
        if (subscriptionInfo.subscription_status === 'expired' || subscriptionInfo.subscription_status === 'cancelled') {
            return {
                type: 'error',
    ...
    return (
        <AnimatePresence>
            ...
            >
                <div className={`bg-gradient-to-r ${notificationContent.gradient} p-1 rounded-xl shadow-xl`}>
