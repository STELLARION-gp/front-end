...
const BlogDetailedPage: React.FC<BlogDetailedPageProps> = ({
  blog,
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const imageHtml = blog.image
        ? `<div style="text-align:center;margin:18px 0;height:480px;display:flex;align-items:center;justify-content:center;overflow:hidden;"><img src="${String(
            blog.image
          )}" class="pdf-image" style="width:100%;height:auto;max-height:480px;object-fit:contain;display:block;margin:0 auto;" /></div>`
        : "";
      ...
      temp.innerHTML = `
        <div style="font-family: Arial, Helvetica, sans-serif; color:#000;">
          ...
          <div style="font-size:14px;color:#444;margin-bottom:12px">By <strong>${safeAuthor}</strong> ${
        blog.createdAt
          ? `• ${new Date(String(blog.createdAt)).toLocaleDateString()}`
          : ""
      }</div>
          ${imageHtml}
      ...
      while (srcY < pxFullHeight) {
        ...
        anchors.forEach((a) => {
          try {
            ...
            if (relTop + r.height < srcY || relTop > srcY + canvasPage.height)
              return; // not on this slice
  ...
  /* Removed unused handleRate function */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (rating > 0) {
        try {
          await blogService.rateBlog(blog.id, rating);
          // Rating state updates removed as they're not used
        } catch (rateErr) {
          console.error(
            "Failed to submit rating during comment submit",
            rateErr
          );
        }
      }
  ...
  const authorName = String(
    (blog as any).author || (blog as any).author_display_name || ""
  );
  const authorAvatar =
    ...
    (authorName &&
      `https://ui-avatars.com/api/?name=${encodeURIComponent(
        authorName
      )}&background=333&color=fff&rounded=true&size=128`) ||
  ...
  return (
    <div className="blog-page">
      <header className="blog-header">
        ...
        <h1 style={{ marginTop: "1rem" }} className="blog-title">
          {blog.title}
        </h1>
        <div className="blog-header-row">
          <div className="blog-meta">
            <span className="blog-author">
              ...
              />
              <Link
                to={`/dashboard/author/${encodeURIComponent(
                  String(blog.author || "")
                )}`}
          ...
          <div className="blog-header-actions">
            ...
            <button
              className={`blog-header-icon${isFavorite ? " favourite" : ""}`}
              onClick={async () => {
                if (!blog?.id) return;
                try {
                  const resp: any = await blogService.toggleBlogLike(
                    blog.id as number
                  );
