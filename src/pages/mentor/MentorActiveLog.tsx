...
import { MagnifyingGlassIcon, ArrowRightIcon, StarIcon } from '@heroicons/react/24/outline';
  ...
  const renderDateCircle = (activity: Activity) => {
    ...
    return (
      <div className="date-circle">
        ...
        <div style={{ fontSize: 12 }}>{date.toLocaleString('default', { month: 'short' })}</div>
      </div>
    );
  };


  const renderDateRangeVertical = (activity: Activity, children: React.ReactNode) => {
      ...
      <div className="date-range-vertical-row activity-row">
        <div className="date-range-vertical">
          <div className="date-range-circle">
            ...
            <div style={{ fontSize: 12 }}>{startDate.toLocaleString('default', { month: 'short' })}</div>
          </div>
          <div className="date-range-vertical-connector" />
          <div className="date-range-circle">
            <div style={{ fontSize: 18 }}>{endDate.getDate()}</div>
            <div style={{ fontSize: 12 }}>{endDate.toLocaleString('default', { month: 'short' })}</div>
          </div>
        </div>
        <div className="activity-card date-range-vertical-card">
          {children}
        </div>
      </div>
    );
  ...
  const renderActivityContent = (activity: Activity) => {
    switch (activity.type) {
      case 'session':
        return (
          <>
            <div style={{ fontWeight: 600, fontSize: '1.08rem', color: '#fff', flex: 1 }}>
              {activity.title}
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 2, marginTop: 8 }}>
                {[1, 2, 3, 4, 5].map(i => (
                  <StarIcon 
                    ...
                    style={{ 
                      width: 18, 
                      color: i <= (activity.rating ?? 0) ? '#fbbf24' : '#cbd5e1',
                      fill: i <= (activity.rating ?? 0) ? '#fbbf24' : 'none'
                    }} 
                  />
                ))}
                {activity.extra && (
                  <span style={{ fontWeight: 700, color: '#60a5fa', marginLeft: 6, fontSize: 18 }}>
                  </span>
                )}
              </div>
            )}
            ...
            {activity.members && (
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: 8, gap: 0 }}>
                <div className="session-members" style={{ justifyContent: 'center' }}>
                  {activity.members.map(member => (
                    <img key={member.id} src={member.avatar} alt={member.name} className="member-avatar" />
                  ))}
                </div>
                <span style={{ fontSize: 22, color: '#60a5fa', marginLeft: 8, fontWeight: 700, display: 'inline-block', verticalAlign: 'middle' }}>+</span>
              </div>
            )}
  ...
  return (
    <div className="mentor-activity-log">
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginBottom: 24 }}>
        <Button onClick={() => navigate('/dashboard/recommended-contents')} variant="primary">
          Recommended Contents
        </Button>
        <Button onClick={() => navigate('/dashboard/recommended-events')} variant="primary">
          Recommended Events
        </Button>
      </div>
      ...
      <div className="activity-filters">
        ...
        <label style={{ fontWeight: 500, fontSize: '1rem', color: '#60a5fa' }}>From</label>
        <input 
          type="date" 
          className="mentor-edit-input" 
          value={from} 
        ...
        />
        <label style={{ fontWeight: 500, fontSize: '1rem', color: '#60a5fa' }}>To</label>
        <input 
          type="date" 
          className="mentor-edit-input" 
          value={to} 
